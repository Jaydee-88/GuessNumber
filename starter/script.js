...
// document.querySelector('.message').textContent = `🎉 Correct sercretNumber!`;
// console.log(document.querySelector('.message').textContent);

// document.querySelector('.sercretNumber').textContent = 13;
...
document.querySelector('.check').addEventListener('click', function () {
  ...
  if (!guess) {
    ...
    // WHEN GUESSS IS TO0 HIGH
  } else if (guess !== sercretNumber) {
    if (score > 1) {
      // document.querySelector('.message').textContent =
      //   guess > sercretNumber ? `📈 Too high!` : `📉 Too low!`;
